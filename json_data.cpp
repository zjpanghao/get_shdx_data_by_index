...
Json::Value get_json_array(const std::string &str) {
  ...
  if (!reader.parse(str, root, false)) {
    std::cout << "--Plz check your url make sure your url is available--"
        << std::endl;
    return 0;
  }
...
std::string get_str_of_time() {
  ...
  char buffer[13] = { 0 };
  sprintf(buffer, "%d%02d%02d%02d%02d", current_time->tm_year + 1900,
...
void get_url_by_toKen(const std::string &url2_toKen,
                      const std::string& url_time_key, std::string& url,
                      int url_index) {
  std::string url1_ip =
      "http://61.129.39.71/telecom-dmp/kv/getValueByKey?token=";
  std::string url3_table = "table=kunyan_to_upload_inter_tab_up&key=";
  std::string url_prefix = url1_ip + url2_toKen + "&" + url3_table
      + url_time_key + "_ky_";
  char str_i[10];
...
std::string get_value_by_url(const std::string &url, std::string& token,
  ...
  if (code == MESSAGE_WRONG_TOKEN) {
    const char* token_url =
        "http://61.129.39.71/telecom-dmp/getToken?apiKey=98f5103019170612fd3a486e3d872c48&sign=6a653929c81a24ba14e41e25b6047e5dec55e76e";
...
bool get_data_from_shanghai_1(std::string& url,
                              std::string& value_base64_decode,
                              std::string& token, CURL *curl) {

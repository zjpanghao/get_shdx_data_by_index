...
int TeleCompress::Uncompressbase64(const char*input, int inlen,
...
int TeleCompress::ReplaceTag(std::string &str, std::string old_value,
...
int TeleCompress::Deflate_gzip(unsigned char *input, int inlen,
                               unsigned char *output, int *outlen) {
  int ret;
...
std::string TeleCompress::GetProcessResult(std::string input) {
  ...
  if (input.length() == 0)
    return result;
  len = Uncompressbase64(input.c_str(), input.length(), base_result,
  MAX_CONTENT_LENGTH);
  if (len <= 0) {
  ...
  len = Uncompressbase64(value.c_str(), value.length(), base_result,
  MAX_CONTENT_LENGTH);
  if (len <= 0) {
